 ...
 * 	Standard, Group, Paypal, Word to Word - English to Danish, 12.00 JPY per word rate
 * @reference https://gengo.testrail.com/index.php?/cases/view/40350
 */
public class C40350_9_OrderUsingJPYCurrency extends AutomationBase{
    ...
    public void initFields() throws IOException{
        ...
        itemToTranslate = new String[] {
                var.getItemToTranslate(56),
                var.getItemToTranslate(57)
        };
        unitCount = new String[] {
                var.getUnitCountSource(56),
                var.getUnitCountSource(57)
        };
        translatedItem = new String[] {
                var.getTranslatedItem(56),
                var.getTranslatedItem(57)
        };
    }

    public C40350_9_OrderUsingJPYCurrency() throws IOException {}
    ...
    public void placeAnOrder() {
        ...
        assertEquals(customerOrderQuotePage.getUnitPriceText().getText().replaceAll("[A-z -.]",""), JPY12);
        ...
        assertEquals(customerCheckoutPage.getUnitPriceText().getText().replaceAll("[A-z /]",""), JPY12);

        // Check if the unit prices for Standard and Business tiers are as expected
        assertEquals(customerCheckoutPage.getStandardUnitPriceText().getText().replaceAll("[A-z /]",""), JPY12 );
        assertEquals(customerCheckoutPage.getBusinessUnitPriceText().getText().replaceAll("[A-z /]",""), JPY18);
