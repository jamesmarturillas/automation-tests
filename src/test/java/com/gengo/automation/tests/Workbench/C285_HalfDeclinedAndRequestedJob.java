...
/**
 * @case Half Declined and Requested Jobs
 * (Word to Character, Stripe Payment, Standard Tier)
 * @reference https://gengo.testrail.com/index.php?/cases/view/285
 */
public class C285_HalfDeclinedAndRequestedJob extends AutomationBase {

    private String parentWindow, orderNo, comment, excerpt, newExcerpt;
    private String[] itemToTranslate, revisedItem, jobNo = new String[4],
            excerpts, excerpt2, excerpt1, unitCount, translatedItem, translatedItem2;
    ...
    public void initFields() throws IOException{
        ...
        translatedItem = new String[] {
                var.getTranslatedItem(17).substring(0,25),
                var.getTranslatedItem(25).substring(0,25)
        };
        revisedItem = new String[] {
        ...
        comment = var.getCustomerCorrection();
    }

    public C285_HalfDeclinedAndRequestedJob() throws IOException {}
    ...
    /**
     * pickUpAndTranslateByTranslator1 --- Translator 1 picks up a collection,
     * translates 2 of the jobs and declines the rest
     * */
    ...
    /**
     * customerRequestCorrection --- this method has the customer request
     * revisions for the previously submitted jobs
     * */
    ...
    public void customerRequestCorrection(){
        ...
        assertTrue(page.getCurrentUrl().equals(customerDashboardPage.CUSTOMER_DASHBOARD_URL),
                var.getWrongUrlErrMsg());
        for(String text: excerpt1) {
            globalPage.goToOrdersPage();
            customerOrdersPage.clickReviewableOption();
            customerOrdersPage.findOrder(text);

            // Revisions are requested for the jobs
            customerOrderDetailsPage.requestCorrection(comment, true, true, true);
        }
        global.nonAdminSignOut();
        assertTrue(homePage.checkHomePage());
    }
    ...
    /**
     * reviseByTranslator1 --- The translator revises the previously submitted jobs
     * and submits them again
     * */
    @Test(dependsOnMethods = "customerRequestCorrection")
    public void reviseByTranslator1() throws AWTException {
        pluginPage.passThisPage();
        assertTrue(homePage.getSignIn().isDisplayed(),
                var.getElementNotFoundErrMsg());
        ...
        // Add the text translated on the workbench
        workbenchPage.addMultipleTranslatedText(revisedItem, excerpt1, revisedItem.length);
        workbenchPage.submitJob();
        workbenchPage.submitModalOk(false);
        global.nonAdminSignOut();
        assertTrue(homePage.checkHomePage());
    ...
    /**
     * pickUpByTranslator2 --- Another translator picks up the job, adds the translations,
     * and submits the collection for review
     * */
    @Test(dependsOnMethods = "reviseByTranslator1")
    public void pickUpByTranslator2(){
        ...
        // Add translations for remaining texts
    ...
    public void customerApprove(){
        ...
        for(String text: excerpts) {
            ...
            if(text.equals(excerpt1[0]) || text.equals(excerpt1[1]))
